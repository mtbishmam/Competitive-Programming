...
int main()
{
    ...
    for (int Ti = 1; Ti <= T; Ti++) {
        ...
        bool cycle = 0; int cc = 0;
        vb vis(n + 1);
        // Using dist array
        // vi dist(n + 1);
        // function<void(int, int)> dfs = [&](int node, int dis) {
        //     vis[node] = 1;
        //     dist[node] = dis;
        //     for (auto& child : g[node]) {
        //         if (!vis[child]) dfs(child, dis + 1);
        //         if (vis[child] and dist[child] < dist[node] - 1) cycle = 1;
        //     }
        //     };
        ...
        function<void(int, int)> dfs = [&](int node, int par) {
            ...
            for (auto& child : g[node]) {
                ...
                else if (vis[child] and child != par) // using if here doesn't work ;)
                    cycle = 1;
            }
            };
        for (int i = 1; i <= n; i++)
            if (!vis[i]) dfs(i, i), cc++;
        if (cc == 1 and !cycle) cout << "YES";
        else cout << "NO";
    }
    return 0;
}