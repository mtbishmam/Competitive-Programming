...
int main()
{
    ...
    for (int Ti = 1; Ti <= T; Ti++) {
        ...
        function<ll(int, int)> f = [&](int i, int j) {
            if (i < 0 or j < 0 or i >= n or j >= m) return 1ll * INF;
            if (i == n - 1) return a[i][j]; // won't find more paths bc there are no neg values 
            auto& ret = dp[i][j];
            if (ret != INF) return ret;
            ret = min(ret, a[i][j] + f(i + 1, j));
            ret = min(ret, a[i][j] + f(i, j + 1));
            ret = min(ret, a[i][j] + f(i + 1, j + 1));
            ret = min(ret, a[i][j] + f(i - 1, j - 1));
            return ret;
            };
        cout << f(0, 0) << endl;
        // for (auto& dpi : dp) cout << dpi << endl;
    }
    return 0;
}